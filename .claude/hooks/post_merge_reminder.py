...
phrase; in the yt-shorts port this hook's own test payload, an `echo` of a JSON
string, injected "a merge just completed" with nothing merged. So the match is
anchored to a COMMAND POSITION.

That is not enough on its own, because a heredoc body is PROSE inside a command
string, and shell syntax does not apply to it: writing this very fix fired the
hook twice in two minutes, off a commit message and a PR body that quoted
